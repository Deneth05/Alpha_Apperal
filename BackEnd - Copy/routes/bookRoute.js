...
router.post('/', async (req, res) => {
    try {
        const { itemCode, productName, productCategory, Quantity, unit, price } = req.body;
        if (!itemCode || !productName || !productCategory || !Quantity || !unit || !price) {
            return res.status(400).send({
                message: 'Send all required fields: itemCode, productName, productCategory, Quantity, Unit, price',
            });
        }

        const newItem = { itemCode, productName, productCategory, Quantity, unit, price };
